...
export const MoneyMomentsScreen: React.FC<Props> = ({ session }) => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      alert('Failed to load money moments')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleCompute = async () => {
    ...
    try {
      ...
      alert(`Computed ${result.count} money moments!`)
      void loadData() // Reload to get updated data
    } catch (err) {
      console.error('Error computing moments:', err)
      alert(err instanceof Error ? err.message : 'Failed to compute moments')
    } finally {
      setComputing(false)
    }
  }
  ...
  const handleProcessNudges = async () => {
    ...
    try {
      ...
      alert(`Evaluated ${evalResult.count} nudge candidates`)
      
      // Then process and deliver
      const processResult = await processNudges(session, 10)
      alert(`Processed and delivered ${processResult.count} nudges!`)
  ...
  const getIcon = (habitId: string) => {
    if (habitId.includes('burn_rate') || habitId.includes('spend_to_income')) {
      return <span className="moment-icon">📈</span>
    }
    if (habitId.includes('micro') || habitId.includes('cash')) {
      return <span className="moment-icon">ℹ️</span>
    }
    return <span className="moment-icon">⚠️</span>
  }
