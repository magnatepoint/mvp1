#!/bin/bash
# Force kill whatever is using port 8000
...
echo -e "${BLUE}🔍 Finding what's using port 8000...${NC}"
echo ""
...
if command -v lsof &> /dev/null; then
    ...
    if [ -n "$PIDS" ]; then
        echo -e "${YELLOW}Found processes using port 8000:${NC}"
        for PID in $PIDS; do
...
if [ -z "$PIDS" ] && command -v netstat &> /dev/null; then
    ...
    if [ -n "$PID" ] && [ "$PID" != "-" ]; then
        echo -e "${YELLOW}Found process using port 8000 (PID: $PID)${NC}"
...
if docker ps --format '{{.Names}}' | grep -qE "(mvp|backend|8000)"; then
    echo -e "${YELLOW}Found Docker containers:${NC}"
    docker ps --format 'table {{.Names}}\t{{.Ports}}' | grep -E "(mvp|backend|8000)"
    echo ""
...
if lsof -ti:8000 > /dev/null 2>&1 || (command -v netstat &> /dev/null && netstat -tlnp 2>/dev/null | grep -q :8000); then
    echo -e "${RED}❌ Port 8000 is still in use${NC}"
    echo -e "${YELLOW}Try running with sudo:${NC}"
    echo "  sudo ./deploy/scripts/kill-port-8000.sh"
    exit 1
else
